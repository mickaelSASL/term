...
def ajout_temp(capteur, temp):
    """
    Met à jour la température enregistrée dans le fichier JSON

    Parameters
    ----------
    capteur : str
        Nom du cpateur à actualiser.
    temp : int
        Valeur de la température à actualiser.
    ...
    """
    json_str = lecture_json()
    json_str[capteur]['valeur']=temp
    with open(JSON_FICHIER, 'w') as mon_fichier:
    	json.dump(json_str, mon_fichier)
