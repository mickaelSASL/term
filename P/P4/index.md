...
!!! Example "Exemple"
    Par exemple, pour les chaînes ABCBDAB et BDCABA, la longueur maximale des PLSS est 4 et l’une d’entre elles est BDAB.
...
Parmis toutes les possibilités, nous identifions la plus longue sous-séquence (ou une des plus longue si il en existe plusieurs de la même taille) commune aux deux chaines.
...
|:--------------------------------- :|: -------------------------- :|: --------------------------------- :|: -------------------------- :|: -------------------------- :|
|                ∅                   |   B<br>A<br>C<br>B<br><br>   |  BA<br>BC<br>BB<br>AC<br>AB<br>CB   |    BAC<br>BAB<br>BCB<br>ACB  |              BACB            |

Il y en a donc 16 différentes (y compris la vide).
___

!!! note "Les questions suivantes portent sur la chaine `BCB`"
...
!!! Example "Exemple"
    `PLSS("ABCBDAB", "BDAB")` est égale à `BDAB` 
    *(voir l’exemple du début de l’exercice).*
...
Afin de pallier à ce soucis on crée un dictionnaire M qui à chaque couple (chaîne1, chaîne2) associe la PLSS. Ce dictionnaire sera une variable « globale » qui pourra donc être lue et modifiée par chacun des appels récursifs.
